...
def kor(x, y, hely) :
    ...
    hely.create_rectangle(korr,korr,mezomeret+korr, mezomeret+korr, width = 2,fill="dark grey",outline="dark blue")
...
def uj() :
    ...
    palyatar = {}
    vege = 0
    for i in range(0, mezoszam):
        for j in range(0, mezoszam):
            palyatar[(i,j)] = 0
    jatekos = 1
    for x in range(0, palyameret, mezomeret):
        for y in range(0, palyameret, mezomeret):
            x1,y1,x2,y2 = x+korr,y+korr,x+mezomeret+korr,y+mezomeret+korr
            canPalya.create_rectangle(x1,y1,x2,y2,width = 2,fill = "dark grey", outline = "dark blue")
    canNyert.create_rectangle(korr,korr,mezomeret+korr,mezomeret+korr,width=2,fill = "dark grey", outline = "dark blue")
    kor(0,0,canKovetkezo)

def palyakattint(event):
   global canKovetkezo, mezomeret, korr, jatekos, palyatar, vege, canNyert
   if vege == 0:
...
def kiertekel(x,y):
    ...
    if db != 5:
        db = 0
    yj = y
    while db < 5 and xj < mezoszam and palyatar[(x,xj)] ==jatekos:
        db, yj = db + 1, yj+1
    yb = y-1
    while db < 5 and yb >= 0 and palyatar[(x,xb)] ==jatekos:
        db,yb=db+1,yb-1
    if db != 5:
        db = 0
    xj,xj = x,y
    while db < 5 and yj < mezoszam and xj < mezoszam and palyatar[(xj,yj)] == jatekos:
        db, xj,yj = db+1,xj+1,yj+1
    xb,yb = x-1,y-1
    while db<5 and yb >=0 and xb >=0 and palyatar[(xb,yb)] == jatekos:
...
canNyert = Canvas(ablak,bg="dark grey",height=mezomeret+korr,width = mezomeret+korr)
canKovetkezo = Canvas(ablak,bg="dark grey",height=mezomeret+korr,width = mezomeret+korr)
cmdButton = Button(ablak,text="Új játék",command=uj,width=15)
canPalya = Canvas(ablak,bg="dark grey",height = palyameret+korr,width = palyameret+korr)
canPalya.grid(row=1,column=1,rowspan=3,padx=15,pady=15)
lblKovetkezo.grid(row=1,column=1)
canKovetkezo.grid(row=1,column=1,padx=15)
...
canPalya.bind("<Button-1>",palyakattint)
uj()
ablak.mainloop()
